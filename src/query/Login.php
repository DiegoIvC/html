...
class Login
{
    public function verificalogin($correo, $contrasena)
    {
        try
        {
            ...
            $cc = new database("instrumentales","root","");
            $objetoPDO = $cc->getPDO();
            ...
            if($pase > 0 )
            {
                ...
                $usuario="SELECT * FROM usuario INNER JOIN roles ON usuario.Roles_id=roles.id INNER JOIN rol ON usuario.rol=rol.cve_rol WHERE Correo='$correo'";
